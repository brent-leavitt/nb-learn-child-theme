	...
	foreach ( $memberships as $membership ) {
		...
		if ( $membership->is_recurring() && $membership->is_expired() && $membership->can_update_billing_card() ) : ?>
			<p class="rcp_error">
				<span>
					<?php
					printf( __( 'Your %s subscription has expired. <a href="%s">Update your payment method</a> to reactivate and renew your subscription.', 'rcp' ),
...
?>
	<div class="rcp-table" id="rcp-account-overview">
		...
		<div class="rcp-account-overview">
			<span><?php _e( 'Subscription', 'rcp' ); ?></span>
				...
				<div class="rcp-account-overview">
					...
					<span data-th="<?php esc_attr_e( 'Actions', 'rcp' ); ?>">
						...
						if ( $membership->can_renew() ) {
							$links[] = apply_filters( 'rcp_subscription_details_action_renew', '<a href="' . esc_url( rcp_get_membership_renewal_url( $membership->get_id() ) ) . '" title="' . esc_attr__( 'Renew your subscription', 'rcp' ) . '" class="rcp_sub_details_renew">' . __( 'Renew your subscription', 'rcp' ) . '</a>', $user_ID );
						}

						if ( $membership->upgrade_possible() ) {
							$links[] = apply_filters( 'rcp_subscription_details_action_upgrade', '<a href="' . esc_url( rcp_get_membership_upgrade_url( $membership->get_id() ) ) . '" title="' . esc_attr__( 'Upgrade or change your subscription', 'rcp' ) . '" class="rcp_sub_details_change_membership">' . __( 'Upgrade or change your subscription', 'rcp' ) . '</a>', $user_ID );
						}

						if ( $membership->is_active() && $membership->can_cancel() && ! $membership->has_payment_plan() ) {
							$links[] = apply_filters( 'rcp_subscription_details_action_cancel', '<a href="' . esc_url( rcp_get_membership_cancel_url( $membership->get_id() ) ) . '" title="' . esc_attr__( 'Cancel your subscription', 'rcp' ) . '" class="rcp_sub_details_cancel" id="rcp_cancel_membership_' . esc_attr( $membership->get_id() ) . '">' . __( 'Cancel your subscription', 'rcp' ) . '</a>', $user_ID );
						}
							...
							<script>
								...
								if ( cancel_link ) {
									...
									cancel_link.addEventListener( "click", function ( event ) {
										...
										var message = '<?php printf( __( "Are you sure you want to cancel your %s subscription? If you cancel, your subscription will expire on %s.", "rcp" ), $membership->get_membership_level_name(), $membership->get_expiration_date() ); ?>';
		...
		<?php else : ?>
			<div>
				<div data-th="<?php esc_attr_e( 'Subscription', 'rcp' ); ?>" colspan="4"><?php _e( 'You do not have any Subscriptions.', 'rcp' ); ?></div>
			</div>
		<?php endif; ?>
		</main>
		</div>
	<div class="rcp-table" id="rcp-payment-history">
		...
		if ( $payments ) : ?>
			<?php foreach ( $payments as $payment ) : ?>
				<div class="rcp-payment-history">
					...
					<span data-th="<?php esc_attr_e( 'Subscription', 'rcp' ); ?>"><?php echo esc_html( $payment->subscription ); ?></span>
		...
		<?php else : ?>
			<div>
				<div data-th="<?php _e( 'Subscription', 'rcp' ); ?>" colspan="6"><?php _e( 'You have not made any payments.', 'rcp' ); ?></div>
			</div>
		<?php endif; ?>
		</main>
		</div>
